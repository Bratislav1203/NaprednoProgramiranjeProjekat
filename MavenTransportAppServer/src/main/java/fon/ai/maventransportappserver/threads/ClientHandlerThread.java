		...
		while (!socket.isClosed()) {
			...
			try {
				...
				case Operation.LOGIN:
					try {
						IGeneralEntity objekat = Controller.getController()
								.pronadjiKorisnika((IGeneralEntity) request.getData());
				...
				case Operation.DELETE_DRIVE:
					...
					sendResponse(response);
					break;
				case Operation.DELETE_COST_ITEM:
					try {
						Controller.getController().obrisiTrosak((IGeneralEntity) request.getData());
						response.setStatus(ResponseStatus.SUCCESS);
					} catch (Exception e) {
						response.setException(e);
						response.setStatus(ResponseStatus.ERROR);
				...
				case Operation.DELETE_COST_LIST:
					try {
						Controller.getController().obrisiListuTroskova((IGeneralEntity) request.getData());
						response.setStatus(ResponseStatus.SUCCESS);
					} catch (Exception e) {
						response.setException(e);
						response.setStatus(ResponseStatus.ERROR);
				...
				case Operation.TAKE_DRIVE_BY_ID:
					try {
						IGeneralEntity objekat = Controller.getController()
								.vratiVoznjuPoIDu((IGeneralEntity) request.getData());
						response.setData(objekat);
						response.setStatus(ResponseStatus.SUCCESS);
					} catch (Exception e) {
						response.setException(e);
				...
				case Operation.TAKE_COST:
					...
					sendResponse(response);
					break;
				case Operation.UPDATE_COST_ITEM:
					try {
						Controller.getController().updateCostItem((IGeneralEntity) request.getData());
						response.setStatus(ResponseStatus.SUCCESS);
					} catch (Exception e) {
						response.setException(e);
						response.setStatus(ResponseStatus.ERROR);
